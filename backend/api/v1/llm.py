...
async def nl_query(req: NLQuery):
    try:
        ...
        rows = run_select(sql)
        try:
            answer = generate_answer_from_rows(req.task, sql, rows, table_hint=req.table_hint)
        except Exception:
            answer = None
        return {"sql": sql, "rows": rows, "count": len(rows), "answer": answer}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
        ...
        for p in parts:
            ...
            for stmt in inner or [p]:
                ...
                try:
                    ...
                    # Попробуем извлечь имя таблицы для снепшота (простые эвристики)
                    tname = None
                    up2 = stmt.upper()
                    if up2.startswith("ALTER TABLE"):
                        # ALTER TABLE "name" ...
                        after = stmt.split("ALTER TABLE", 1)[1].strip()
                        if after.startswith('"'):
                            tname = after.split('"', 2)[1]
                        else:
                            tname = after.split()[0]
                    elif up2.startswith("UPDATE"):
                        after = stmt.split("UPDATE", 1)[1].strip()
                        tname = after.split()[0].strip('"')
                    if tname:
                        try:
                            snapshot_table(tname, operation="llm_write", details=stmt[:500])
                        except Exception:
                            pass
                except Exception as e:
...
async def agent_query(req: NLQuery):
    ...
    """
    try:
        result = agent_loop(req.task, table_hint=req.table_hint, max_iterations=3)
        return result
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

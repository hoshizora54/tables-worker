...
class PlotRenderRequest(BaseModel):
    ...
    limit: int = 100
    bins: int = 20
    where: Optional[str] = None  # простой SQL-фильтр, доверяем бэкенду
    title: Optional[str] = None


@router.post("/render")
async def render_plot(req: PlotRenderRequest):
    try:
        t = req.table
        chart = (req.type or "").lower()
        if chart in ("bar", "line"):
            x = req.x or _pick_column_for_x(t, NLPlotRequest(task="", table_hint=t))
            y = req.y
            agg = (req.agg or "count").lower()
            where = f" WHERE {req.where} " if req.where else ""
            if not y and agg == "count":
                sql = (
                    f'SELECT "{x}" AS x, COUNT(*) AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
                )
            elif y:
                if agg == "mean":
                    sql = f'SELECT "{x}" AS x, AVG("{y}") AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
                elif agg == "sum":
                    sql = f'SELECT "{x}" AS x, SUM("{y}") AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
                elif agg == "min":
                    sql = f'SELECT "{x}" AS x, MIN("{y}") AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
                elif agg == "max":
                    sql = f'SELECT "{x}" AS x, MAX("{y}") AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
                else:
                    sql = f'SELECT "{x}" AS x, COUNT("{y}") AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
            else:
                sql = f'SELECT "{x}" AS x, COUNT(*) AS y FROM "{t}" {where} GROUP BY "{x}" ORDER BY y DESC LIMIT :lim'
            rows = run_select(sql, params={"lim": req.limit})
            import pandas as pd
            df = pd.DataFrame(rows)
            img = render_plot_to_base64(chart_type=chart, data=df, x="x", y="y", hue=req.hue, title=req.title)
            return {"image_base64": img, "rows": rows}

        if chart == "hist":
            x = req.x or _pick_numeric_column(t) or _pick_any_column(t)
            where = f" WHERE {req.where} " if req.where else ""
            rows = run_select(f'SELECT "{x}" AS x FROM "{t}" {where} LIMIT 5000')
            import pandas as pd
            df = pd.DataFrame(rows)
            img = render_plot_to_base64(chart_type="hist", data=df, x="x", bins=req.bins, title=req.title)
            return {"image_base64": img, "rows": rows, "bins": req.bins}

        if chart == "scatter":
            x = req.x or _pick_any_column(t)
            y = req.y or _pick_numeric_column(t) or _pick_any_column(t)
            where = f" WHERE {req.where} " if req.where else ""
            rows = run_select(f'SELECT "{x}" AS x, "{y}" AS y FROM "{t}" {where} LIMIT :lim', params={"lim": req.limit})
            import pandas as pd
            df = pd.DataFrame(rows)
            img = render_plot_to_base64(chart_type="scatter", data=df, x="x", y="y", hue=req.hue, title=req.title)
            return {"image_base64": img, "rows": rows}

        raise HTTPException(status_code=400, detail=f"Неизвестный тип графика: {req.type}")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
